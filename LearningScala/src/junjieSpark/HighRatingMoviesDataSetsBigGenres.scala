...
object HighRatingMoviesDataSetsBigGenres {
    ...
    val genreName = "War"
     ...
     for (line <- lines) {
       if (line.contains("\"")){
         ...
         if (prefields(1).split("\\|").contains(genreName))
           movieNames += (fields(0).toInt -> fields(1))
       }
       else {
         var fields = line.split(',')
         if (fields(2).split("\\|").contains(genreName))
           movieNames += (fields(0).toInt -> fields(1))
...
//    val fields = s.split("\t")
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession
      .builder
      .appName("HighRatingMoviesGenres")
      .master("local[*]")
...
//    val lines = spark.sparkContext.textFile("../ml-100k/u.data").map(parseMovie)
    ...
    val movieCount = lines.reduceByKey((x,y) => (x._1+y._1, x._2+y._2)).filter(x => x._2._2 > 10000)
    .filter(x => names.contains(x._1))
