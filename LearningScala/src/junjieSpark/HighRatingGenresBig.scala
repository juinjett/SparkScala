...
//  def parseValue(rating: Double, s: String) = {
//    var res: Option[(Double, String)] = None
//    val fields = s.split(',')
////    val fields = s.split("\t")
...
//  }
  ...
  def main(args: Array[String]) {
    ...
    val lines = spark.sparkContext.textFile("../ml-20m/ratings.csv").map(x => (x.split(',')(2).toDouble, genresMap(x.split(',')(1).toInt)))
//    for (each <- lines) println(each)

    val genres = lines.flatMapValues(x => x.split("\\|")).collect.take(10)
////    val genres = lines.flatMapValue(parseValue).map((_, 1)).reduceByKey(_+_)
