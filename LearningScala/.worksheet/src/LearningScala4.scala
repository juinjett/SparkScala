...
val list = List.range(1,21);System.out.println("""list  : List[Int] = """ + $show(list ));$skip(51); 
for (x <- list) {
	if(((x % 3)&1) == 0) println(x)
};$skip(51); 

val newlist = list.filter( x => ((x%3)&1) == 0);System.out.println("""newlist  : List[Int] = """ + $show(newlist ));$skip(30); 
for (x <- newlist) println(x)}
